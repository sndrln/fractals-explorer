vec3 core_logic(vec2 uv) {
  ...
  vec2 c = mix(coord, seed, juliaMorph); // The Nova constant
  ...
  for (float i = 0.0; i < 1000.0; i++) {
    ...
    // Nova Update Rule: zNext = z + (relaxation * displacement) + c + memory
    vec2 relaxationStep = complexMul(relaxationVec, displacement);
    vec2 inertia = complexMul(zPrev, memFactor);
    vec2 zNext = z + relaxationStep + c + inertia;
